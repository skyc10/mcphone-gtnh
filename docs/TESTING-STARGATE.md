...
- [ ] 2026-09-18 起含网络同步变更（`StargateSync` id19、`PlayTimeSync` 追加服务器时长尾字段）：**客户端与服务端必须同为本次构建**，才能完整测 §1.1/§1.3 的「被禁 App 隐藏」与「服务器时长」。只有客户端是新版时的安全退化：被禁 App 照旧显示（点击弹星门提示）、服务器时长显示「同步中」。
...
- [ ] **默认配置**：主屏**不显示** ME App 图标，常显 HUD 同步不显示（`StargateSync` 名单过滤；改配置后 ≤20 秒或重进世界生效）。
- [ ] （退化场景：名单未同步，如旧服务端）**未安装 AE2**：打开手机 ME App → 提示星门规则已关闭手机注册（`[ae2] registerWirelessTerminal=false`），不崩溃。
- [ ] （退化场景）**安装 AE2 + 默认配置**：打开手机 ME App → 明确提示被星门规则关闭，**不**出现「未检测到 AE2」的误导文案。
- [ ] 潜行 + 持手机右击 ME 安全站 → 提示「AE2 终端功能已被服务器规则（星门）关闭，无需绑定」（该兜底提示保留）。
- [ ] **混搭档**（仅把 `builtinTerminal=true`，`registerWirelessTerminal` 仍 false）：任一终端路径可用 → ME App 图标**仍显示**；打开后仍提示星门关闭注册（修复后不再误报「未检测到 AE2」），且不会打开内置终端。
...
- [ ] **默认配置**：主屏**不显示**末影箱 App 图标，常显 HUD 同步不显示。
- [ ] （退化场景：名单未同步）打开手机末影箱入口 → 服务端提示「末影箱功能已被服务器规则（星门）关闭…[enderchest] enabled」，不打开界面。
...
- [ ] 联动App 页（应用商店 → 联动App）：行与行不再重叠，行内「App 名」与「需要 XXX」两行清晰分离，右侧「已装/未装」贴右沿。
- [ ] 主屏：一行 4 个图标第 4 个完整可见；「应用管理」「应用商店」等长名**不折行**（放不下时出省略号），各行图标垂直对齐。
- [ ] 时钟页：两行为「玩家总游玩时长」（跨会话累计，不再是刚进服的 0）与「服务器时长」（本次服务器运行时长；未同步显示「同步中」）。
- [ ] 调整 GUI 缩放/窗口大小后，以上布局均不裁切、不横向溢出。
