# 星门规则合规说明（v1.0.4 起默认档）

本文档说明 mcphone（GTNH 移植版）针对 GTNH「星门规则」（Stargate Rules）做的合规化设计与配置方法，供玩家、服主与 GTNH staff 审阅。

- 适用版本：**v1.0.4 及以后**——原 `stargate-rules` 分支已并入 `master`，两条构建线合并为同一个版本号；本档位即 v1.0.4 起的**默认档位**（不再需要切分支）
...
| 🌀 传送（多传送点） | ⚠️ 受限玩法，默认收敛 | 规则第 2 条允许「用类似方式传送到路径点」，但自由档（旧 master，即放开配置后的状态）的「无限传送点 + 跨维度 + 免冷却 + 不消耗传送宝石」超出官方 QoL 供给水平。本版默认 **3 个传送点、30 秒冷却、禁止跨维度**，且预留了消耗传送宝石的开关；全部可通过配置调整 |
...
> 想**关闭**这些星门限制（恢复自由档）？README 的「[如何关闭星门规则配置](../README.md#如何关闭星门规则配置恢复自由档)」一节有可直接照抄的配置。
...
| 项 | 自由档（旧 master，配置放开后） | 星门合规档（v1.0.4 起默认） |
| --- | --- | --- |
...
> 2026-09-18 追加：本档位另含三处 UI 修复/调整——主屏标签单行化（长名不再折行导致图标错位）、联动App 页几何随面板归一化（修文字与行重叠）、时钟页时长口径改为「玩家总游玩时长 / 服务器时长」。以上全部随 **v1.0.4** 一并并入 `master`；待测项见 `docs/TESTING-STARGATE.md` §1.5。
